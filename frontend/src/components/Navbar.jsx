    ...
    <header className="sticky top-0 z-40 border-b border-line backdrop-blur-xl bg-navy/70">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="hidden sm:block text-sm text-muted">{profile?.name || profile?.email}</span>
          {profile?.photoURL ? (
            <img src={profile.photoURL} alt="" className="w-8 h-8 rounded-full border border-line" />
          ) : (
